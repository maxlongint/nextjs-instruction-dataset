...
export default function DataInitializer({ children }: DataInitializerProps) {
    ...
    useEffect(() => {
        if (process.env.NODE_ENV === 'development' && isInitialized) {
            // 在控制台提供数据管理工具
            (window as unknown as Record<string, unknown>).dataTools = {
                // 切换数据场景
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-gray-50">
                <div className="max-w-md w-full bg-white rounded-lg shadow-md p-6">
                    ...
                    <h3 className="text-lg font-medium text-gray-900 text-center mb-2">初始化失败</h3>
                    <p className="text-sm text-gray-600 text-center mb-4">{initError}</p>
                    <button
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-gray-50">
                <div className="text-center">
                    ...
                    <h3 className="text-lg font-medium text-gray-900 mb-2">正在初始化数据...</h3>
                    <p className="text-sm text-gray-600">首次启动需要加载模拟数据，请稍候</p>
                </div>
            </div>
        );
    }
